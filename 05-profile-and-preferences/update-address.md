...
The summary card shows the full structured address on file: **House number**, **City**, **State**, **ZIP Code**, **Country** (e.g., 3049 INDEPENDENCE DR, LIVERMORE, California, 94551, United States). Verify it matches your records. If it's already correct, back out without editing.

![Step 4: Review Current Address](../.gitbook/assets/Screenshot_1776963826.png)

#### Step 5: Tap Edit and Open the Edit Form

Tap **Edit**. The structured edit form opens with the processing-note *"Please allow one business day for your address change to be processed."* Six fields appear in order: **House number** (pre-filled), **Street address** (blank), **Country** (United States), **City** (pre-filled), **State** (dropdown — *California(CA)* pre-selected), **ZIP Code** (pre-filled).

![Step 5: Tap Edit and Open the Edit Form](../.gitbook/assets/Screenshot_1776963820.png)

#### Step 6: Update Fields and Save

Edit whichever fields are changing. The **State** dropdown covers all 50 states (plus territories where applicable). When all fields are accurate, tap **Save**. The change triggers a core-system sync and any downstream address-dependent services (card reissuance, statement mailing) pick up the new address on their next scheduled event.

![Step 6: Update Fields and Save](../.gitbook/assets/Screenshot_1776963828.png)

### Summary

Address updates propagate slowly by design — the core takes a few minutes, and downstream services (card reissuance mailing, statement mailing) can take days to pick up the change. Because of that cascade, members changing address just before a card reissuance should confirm with support that the card mail goes to the new address. The structured-field design (house number / city / state / ZIP / country) forces clean data entry instead of a free-text address that would need parsing downstream. The explicit *"allow one business day"* note sets expectations so you don't check too soon and think the update didn't apply.
...
* Member moves across state lines: Edit → change all fields including State dropdown → Save.
...
* Address is already correct on file: back out — no action needed.
