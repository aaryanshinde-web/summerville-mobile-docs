...
_Summerville Mobile › Accounts › Account Settings & Beneficiaries_

## Accounts: Account Settings & Beneficiaries
...
Account Settings is the everyday tuning surface — name, visibility, default transfer source, sneak-peek preferences. Membership Settings is the read-only ownership view — primary members and beneficiaries stay exactly as they're on file in the core, because both are frequent fraud targets and regulatory expectations require signed changes. If a member asks "how do I add a beneficiary," the answer from this screen is clear: contact a branch or send a secure message via Help → Connect With Us.

### Key Use Cases

* Rename the default Retail Checking to something friendlier ("Joint Checking"): Account settings → Rename.
* Hide a legacy account that's empty but not closed: Account settings → Show/Hide toggle off.
* Confirm joint-ownership on a membership: Membership settings → Primary members.
* Add or change a beneficiary: Membership settings explains the policy — call support or send a secure message.
